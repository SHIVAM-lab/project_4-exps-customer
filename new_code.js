 ...
 async function get_all_customers(req, res) {
        if (typeof req.body.page !== 'undefined' && typeof req.body.limit !== 'undefined') {
            ...
            try{
                ...
                var records=await query_exec("SELECT "+aColumns+" FROM `ep_quotes`  "+join+"  " + sWhere);
                if(filtered_records) filtered_records=records.length;
                res.setHeader('Content-Type', 'application/json');
                res.status(200).send(`{"status":"1","data":` + JSON.stringify(result) + `,"message":"Success","recordsTotal":"`+total_records+`","recordsFiltered":"`+filtered_records+`"}`);
